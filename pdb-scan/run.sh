...
npdb_max=20         # max number of items considered
#
...
while $ok; do
  ...
  if [ $do -ge 0 ]; then
    ...
    if [ $do -ne 2 ]; then echo -ne "X"; mv $list_file list.tmp; grep -v "$(echo $id | awk '{print toupper($0)}')" list.tmp > $list_file; rm -rf $localdir list.tmp *.gz
    else echo -ne "."; mv *.gz $localdir/; cd $localdir; gunzip *.gz
      mv pdb${id}.ent ${id}.pdb; $bindir/cif2mtz.sh r${id}sf.ent ${id}.mtz >/dev/null; mv r${id}sf.ent ${id}.cif; gzip ${id}.*; cd ../; npdb=` expr $npdb + 1`; fi; fi
  # REFMAC RUNS
  if [ -d $localdir ]; then 
